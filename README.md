...
후속 방향은 [ADR-0007](docs/adr/0007-satsa-coverage-failure-response.md)에서 결정했습니다. 869 CpG 트랙의 SATSA 검증은 수행 불가로 종결하고, SATSA에 실제로 존재하는 610개 CpG만 사용하는 별도 모델 쌍을 학습 데이터만으로 새로 잠가 **판정 기준을 바꾸지 않은 채** 재검증합니다.

판정표와 상세 수치는 [SATSA beta 스트리밍 추출과 품질 게이트](docs/satsa-beta-extraction.md)에 있습니다.
