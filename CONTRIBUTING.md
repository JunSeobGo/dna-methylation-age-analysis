...
다음처럼 이후 분석 결과와 구현 방향에 지속적인 영향을 주는 결정은 `docs/adr/`에 기록합니다.

- 학습·검증 데이터의 역할과 재사용 금지 정책
- 교차검증 단위, 누수 방지와 결측 처리 원칙
- 모델 후보 채택·잠금·교체 조건
- 대용량 데이터 저장·추출 방식
- 기존 재현 절차와 호환되지 않는 구조 변경

단순 오탈자, 작은 버그 수정, 일회성 탐색 결과는 ADR 대상이 아닙니다. 승인된 결정을 바꿀 때는 이전 ADR을 덮어쓰지 않고 새 ADR에서 대체 이유를 기록합니다. 공개 분석 결정은 `docs/adr/`, 개인 회고와 면접용 서술은 Git에서 제외된 `portfolio-private/`에 둡니다.
